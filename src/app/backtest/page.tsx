...
import { api, selectSymbols, SITE, socket, symbols } from "@/src/utils/constants";
...
import { formatter, genMeta } from "@/src/utils/funcs";
    ...
    return (
        <><TuMeta title={`Backtest - ${SITE}`}/>
            <div className=" w-100p h-100p relative md:p-5 p-2 flex flex-col">
                ...
                <div
                    ref={paramsAreaRef}
                    className="p-4 border-1 border-card br-10 params-area bg-base-100 shadow-lg"
                >
                    <div className="flex justify-between items-center w-100p mb-2">
                        <span>{formData?.symbol?.value.toString()}</span>
                        <button
                            onClick={onCtrlBtnClick}
                            className="ctrl-btn btn btn-primary mb-2"
                        >
