...
const UserBotsPage: FC<IProps> = ({ bots, error }) => {
    ...
    return error ? (
        ...
        <>
            <TuMeta title={`${router.query}'s bots - ${SITE}`} />
            <div className="p-5">
...
async function getData(username: string){
    ...
    return {data: res.data}
        ...
        return {err: {code: err?.response?.statusCode ?? 500 , msg: err?.response?.data?.replace("tuned:", "") ?? "Something went wrong"}}
         
    }
}
export default async function page({params: {username}}){
    const {err, data} = await getData(username)
    return  <Page err={err} bots={data} {...{username}}/>
}
